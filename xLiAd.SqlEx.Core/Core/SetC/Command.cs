...
namespace xLiAd.SqlEx.Core.Core.SetC
{
    ...
    public abstract class Command<T> : ICommand<T>, IInsert<T>
    {
        ...
        #endregion
        private async Task<int> ExecAsync(string sqlString, Dictionary<string, object> param, IDbTransaction dbTransaction)
        {
            try
            {
        ...
        private int Exec(string sqlString, Dictionary<string, object> param, IDbTransaction dbTransaction)
        {
            try
            {
